...
int32_t AppIMPDataProcess(int32_t * const pData, uint32_t *pDataCount)
{
  ...
  // iImpCar_Type * pDataRealImage = (iImpCar_Type*)pData;
...
//   Đối với mỗi phần tử trong mảng dữ liệu (pData) có kích thước DataCount:
// Thực hiện AND bit với giá trị 0x3ffff để giữ lại 18 bit cuối cùng.
// Kiểm tra bit thứ 17 (bit đánh dấu dấu).
// Nếu bit thứ 17 bằng 1 (âm):
// Thực hiện OR bit với giá trị 0xfffc0000 để chuyển sang dạng bù hai của 18 bit, trong đó bit thứ 17 là bit dấu.
  for(uint32_t i=0; i<ImpResCount; i++)
  {
    ...
    else
    {
     ...
     RzPhase = (RcalPhase + RzPhase - RloadPhase - RzRloadPhase);
      ...
      // pSrcData[i].Real = RzReal;
      // pSrcData[i].Image = RzImage;
      // pDataRealImage[i].Real = RzReal;
      // pDataRealImage[i].Image = RzImage;
    }
  }
